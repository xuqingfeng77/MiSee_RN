    ...
    return (
      <View style={styles.container}>
        ...
        <View style={[styles.row, styles.flex_1]}>
          <View style={[styles.flex_1, styles.left_pannel] }>
              {left}
          </View>

          <View style={[styles.flex_1, styles.right_pannel] }>
              {right}
          </View>
    ...
    for(var i in data){
      ...
      if(index === tabSelected){
        for(var k in data[i]){
          ...
          leftPannel.push(
              <Text  key={"renderLeft"+txtIndex} onPress={this.leftPress.bind(this, i, k)}
        ...
        for(var k in data[i]){
          if(this.state[prefixType + i + '_' + k]){
            for(var j in data[i][k]){
              txtIndex=txtIndex+1;
              rightPannel.push(
                <Text  key={"renderRight"+txtIndex} onPress={this.props.click.bind(this,data[i][k][j])} style={styles.left_row}>{data[i][k][j]}</Text>);
            }
            break;
          }
        }
